...
const createUser = asyncHandler(async (req, res) => {
  ...
  if (!findUser) {
    ...
    const data = {
      to:email,
      subject:"Welcome to our Uyir Organic",
      text:"You have successfully signed in to the Uyir Organic website.",
      htm:`<h3>Welcome You have successfully added your account! ${newUser.firstname}</h3>`
    }
...
const createOrder = async (req, res) => {
  ...
  console.log("triggered", req.user)
  const { _id } = req.user;
  ...
  try {
    ...
    for (const item of orderItems) {
      ...
      if (newQuantity < 0) throw new Error(`Not enough stock for ${product.title}`);
...
const sendLowStockEmail = async (lowStockProducts) => {
  console.log("lowstock triggered")
 let transporter = nodemailer.createTransport({
  ...
  let productDetails = lowStockProducts
    .map((p) => `Product: ${p.name}, ID: ${p.productId}, Remaining Stock: ${p.remainingStock}`)
    .join("\n");

  let mailOptions = {
    from: process.env.MAIL_ID,
    to: process.env.MAIL_ID, // Replace with the recipient email
